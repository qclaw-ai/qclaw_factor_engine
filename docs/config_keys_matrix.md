...
| `factor_engine.skip_if_artifact_record_exists` | `src/factor_engine/factor_engine_runner.py` -> `run_factor_engine()` | 幂等跳过：批量按 `yearly_parquet` 覆盖；日更入口按是否存在 `factors.parquet`（未配置时默认 true） |
| `factor_engine.yearly_parquet_enabled` | `src/factor_engine/factor_engine_runner.py` -> `run_factor_engine()` | 非日更须 `true`，否则退出 |
| `factor_engine.daily_parquet_bundle_enabled` | `src/factor_engine/factor_engine_runner.py` -> `run_factor_engine()` | 日更写多因子 `factors.parquet` + manifest |
| `factor_engine.stage` / `factor_engine.is_rebase` / `factor_engine.compute_workers` | `src/factor_engine/factor_engine_runner.py` -> `run_factor_engine()` | 写入阶段、重算标记、并行 worker 数 |
...
| `backtest.factor_output_dir` | `src/backtest_core/backtest_core_runner.py` -> `run_backtest()` | 历史占位；主链路须 `use_factor_value_files=true` 读 `yearly_parquet` |
| `backtest.use_factor_value_files` | `src/backtest_core/backtest_core_runner.py` -> `run_backtest()` | 是否从 `factor_value_files` 选 `yearly_parquet` 路径（默认 true） |
...
| `factor_corr.use_factor_value_files` | `src/factor_corr/factor_corr_matrix.py` -> `run_factor_corr_matrix()` | 是否从 `factor_value_files` 选 `yearly_parquet` 路径（默认 true） |
