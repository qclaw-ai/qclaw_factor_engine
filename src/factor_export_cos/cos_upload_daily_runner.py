...
from factor_export_cos.cos_upload import run_upload_daily

logger = setup_logger("cos_upload_daily_runner", "logs/cos_upload_daily_runner.log")


def main() -> None:
    parser = argparse.ArgumentParser(
        description="每日增量上传本月 factor+label+meta 到 COS（内网 endpoint 友好）"
    )
    ...
    parser.add_argument(
        "--universe",
        help="领域，例如 ZZ500（默认使用 CLI 或调度脚本传入；config.ini 通常不配置）",
    )
    parser.add_argument(
        "--month",
        help="目标月份 YYYY-MM，默认取当前月",
    )
    parser.add_argument(
        "--region",
    ...
    parser.add_argument(
        "--endpoint",
        help=(
            "COS endpoint（内网推荐 *.cos-internal.*，默认读 "
    ...
    logger.info(
        "runner start config_file=%s section=%s universe=%s month=%s strict=%s",
        args.config,
        section,
        args.universe,
        args.month,
    ...
    logger.info(
        "runner resolved local_root=%s cos_root=%s bucket=%s region=%s endpoint=%s workers=%s universe=%s month=%s strict=%s",
        local_root,
        cos_root,
        bucket,
        region,
        endpoint,
        workers,
        str((args.universe or "ZZ500")).strip(),
        month,
        bool(args.strict),
    )
    ...
    run_upload_daily(
        ...
        region=str(region),
        endpoint=endpoint,
        universe=str((args.universe or "ZZ500")).strip(),
        month=month,
        workers=int(workers),
