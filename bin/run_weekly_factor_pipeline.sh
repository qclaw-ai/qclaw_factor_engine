...
python src/factor_engine/factor_engine_runner.py >> "${LOG_FILE}" 2>&1


# 2) 回测 IO：调用 backtest_core 计算指标，并写入 factor_backtest + 回测 JSON
python src/backtest_io/backtest_io_runner.py >> "${LOG_FILE}" 2>&1


# 3) 选入库：根据阈值打 pass_standard，并同步 factor_basic.is_valid / factor_files.backtest_json_path
python src/selection_and_store/selection_and_store_runner.py >> "${LOG_FILE}" 2>&1


# 4) 相关性：对 is_valid=true 的因子计算 corr matrix 写入 Redis
python src/factor_corr/factor_corr_matrix.py >> "${LOG_FILE}" 2>&1
...
if [ "${SYNC_LEGACY_PATHS}" = "1" ] || [ "${SYNC_LEGACY_PATHS}" = "true" ]; then
  python src/backtest_io/sync_factor_values_path_runner.py >> "${LOG_FILE}" 2>&1
fi
